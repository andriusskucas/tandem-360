...
(function(factory){
                ...
                instance: {
                  ...
                  */
                  ...
                  down: function(e, x, y, ev){
                    ...
                    if(ev.target.className.indexOf("hotspot") > -1){

                        tandem_360_display_hot_info(t,ev);
                        return;
                    }
                  ...
                  frameChange: function(e, nil, frame){
                    ...
                    tandem_360_display_hotspots(t,frame);
                  },
  ...
  function error(message){ try{ console.error('[ Reel ] '+message) }catch(e){} }
    ...
    // tandem 360 plugin javascript start
  function tandem_360_display_hotspots(el,frame){
      //console.log(frame);
      el = $(el);
      if(!el.data('allowhotspots')) return;
      var hotspots = el.data('hotspots');
      hotspots = eval("(" + hotspots + ")");
      var frames = el.data('frames');
      var deg = 360 * frame / frames * -1;
      var deg2 = el.data('mainrotate');
      var id = el.attr('id');

      if(el.parent().find('.info-block').length){
            el.parent().find('.info-block.active').fadeOut(300,function(){
                el.parent().find('.info-block.active').remove();
            });
        }

        el.parent().find('#hotspots').remove();

        var width = el.width() / 2;
        var height = el.height() / 2;

        var spotsHtml = '<div id="hotspots" style="transform:rotateX('+(deg2)+'deg);"><div id="hotspots_holder" style="transform:rotateY('+(deg)+'deg);">';


        for(var i = 0; i < hotspots.length; i++){
            var hott = hotspots[i];
            var tX = width * hott.x / 100;
            var tZ = width * hott.z / 100;
            var tY = height * hott.y / 100;

            var bounds = hott.frames;
            var extraclass = hott.extraclass;
            for(var j =0; j < bounds.length; j++){

                if(frame >= parseInt(bounds[j].min) && frame <= parseInt(bounds[j].max)){
                    spotsHtml += '<div class="hotspot '+extraclass+'" id="hs-'+id+'-'+i+'" data-iii="'+i+'"  style="transform:translateZ('+tZ+'px) translateX('+tX+'px) translateY('+tY+'px) rotateY('+(deg * -1)+'deg);"></div>';
                }
            }
        }
        spotsHtml += '</div></div>';

        el.parent().append(spotsHtml);
    }

    function tandem_360_display_hot_info(el,spot){
        el = $(el);
        var id = spot.target.id;
        if(el.parent().find('.info-block').length){
            el.parent().find('.info-block.active').fadeOut(0,function(){
                el.parent().find('.info-block.active').remove();
            });
        }
        var ind = $('#'+id).data('iii');
        ...
        var xx = el.offset().left;
        var yy = el.offset().top;
        var x = $('#'+id).offset().left - xx;
        var y = $('#'+id).offset().top - yy;
        ...
        var block = $('<div class="info-block active"><h4>'+title+'</h4>'+desc+'</div>');

        block.css({top:y, left: x, display: 'none','max-width':width - $('#'+id).offset().left}).appendTo(el.parent()).fadeIn(400);
...
jQuery(document).ready(function($){
    ...
    // initialize and start reel plugin
    function tandem_360_start_reel(ell){
        var el = ell;
        var id =  el.attr('id');
            var images = el.data('images').split(',');
            var speed = el.data('speed');
            var wheelable = el.data('wheelable');
            var image = el.attr('src');
            var autostart = el.data('autostart');
            if(!autostart) speed = 0;
            ...
            jQuery('#'+id).reel({
                ...
                wheelable: wheelable,
                responsive: true,
                delay: 2 // delay before starting autorotation

            });
    }


    // check if there are 360object on the page
    $('img.tandem-360-reel').each(function(i,ell){
        var el = $(ell);


        var onclick = el.data('playonclick');

        // if object is set to only load when clicked
        if(onclick){

           el.click(function(){
                el = $(this);
               el.unbind();
               tandem_360_start_reel(el);
               el.removeClass('tandem-360-reel-click-to-play');
            }).addClass('tandem-360-reel-click-to-play');
        }else{
            // else load 360 object
            tandem_360_start_reel(el);
        }
